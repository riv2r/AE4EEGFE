...
class MlpNet(nn.Module):
    ...
    def __init__(self,layer_size,device=torch.device('cpu')):
        ...
        self.device = device
        layers=[]
        for layer_Idx in range(len(layer_size)-1):
            if layer_Idx == len(layer_size)-2:
                layers.append(
                    nn.Sequential(
                        nn.BatchNorm1d(num_features=layer_size[layer_Idx],affine=False),
                        nn.Linear(layer_size[layer_Idx],layer_size[layer_Idx+1])
                    )
                )
            else:
                layers.append(
                    nn.Sequential(
                        nn.Linear(layer_size[layer_Idx],layer_size[layer_Idx+1]),
                        nn.Sigmoid(),
                        nn.BatchNorm1d(num_features = layer_size[layer_Idx+1],affine=False)
                    )
                )
